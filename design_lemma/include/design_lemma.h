...
ColouredPartition ImprimitiveToColouring(RelationalStructure X1,
                                         RelationalStructure X2,
                                         std::deque<std::deque<int>> part) {
  ...
  for (int x : X1.domain())
    if (std::find(X2.domain().begin(), X2.domain().end(), x) ==
        X2.domain().end())
      B.push_back(x);
...
template <typename T>
Iso DesignLemma(RelationalStructure X, RelationalStructure Y, double alpha,
                T f) {
  int n = X.domain().size();
  int k = X.arity();
  IsoJoiner J;
  for (int i = 0; i < k; ++i) {
    for (const auto Sx : all_different_tuples(n, i)) {
      ...
      RelationalStructure X3 =
          X2.skeletalSubstructure(2, X2.relation(cx).vertices());
      auto P = X3.witnessOfImprimitivity();
      if (P.first > 0) {
        ColouredPartition Cx = ImprimitiveToColouring(
            X, X3,
            P.second.template partitioning<std::deque<std::deque<int>>>());
        // Cx.equipartition();
        for (const auto Sy : all_different_tuples(n, i)) {
          RelationalStructure Y2 = Y;
          Y2.individualise(Sy);
          ...
          if (cy >= 0) {
            RelationalStructure Y3 =
                Y2.skeletalSubstructure(2, Y2.relation(cy).vertices());
            auto Q = Y3.witnessOfImprimitivity();
            if (Q.first > 0) {
              ColouredPartition Cy = ImprimitiveToColouring(
                  Y, Y3,
                  Q.second
                      .template partitioning<std::deque<std::deque<int>>>());
              // Cy.equipartition();
              J.join(f(Cx, Cy));
            }
          }
