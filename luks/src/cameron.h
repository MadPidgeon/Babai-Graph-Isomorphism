...
public:
	...
	Iso operator() ( Group H, string x, string y ) {
		...
		std::cout << "\"For a moment, nothing happened. Then, after a second or so, nothing continued to happen.\"" << std::endl;
...
template<typename T>
Iso CameronIdentification( RestrictedNaturalSetAction phi, string x, string y, T ) {
