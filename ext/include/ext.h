...
template <typename U, typename V> bool Either<U, V>::isFirst() const {
  return which == 0;
}

template <typename U, typename V> bool Either<U, V>::isSecond() const {
  return which == 1;
}

template <typename U, typename V> const U &Either<U, V>::getFirst() const {
  return first;
}

template <typename U, typename V> const V &Either<U, V>::getSecond() const {
  return second;
}

template <typename U, typename V> Either<U, V>::Either(U u) {
  which = 0;
  new (&first) U(u);
}

template <typename U, typename V> Either<U, V>::Either(V v) {
  which = 1;
  new (&second) V(v);
}
...
template <typename U, typename V> Either<U, V>::~Either() {
  if (isFirst())
    first.~U();
  else
    second.~V();
...
template <typename T> std::vector<T> to_vector(const std::set<T> &s) {
  std::vector<T> r;
  r.reserve(s.size());
  for (const auto &x : s)
    r.emplace_back(x);
...
template <int _r> class all_arrays {
  int _n;
...
template <int _r> all_arrays<_r>::iterator::iterator(int n) : _n(n) {
  _array.fill(0);
}

template <int _r> all_arrays<_r>::iterator::iterator(const self_type &other) {
  _n = other._n;
  _array = other._array;
}

template <int _r>
typename all_arrays<_r>::iterator::self_type
all_arrays<_r>::iterator::operator++(int) {
  self_type i = *this;
  ++(*this);
  return i;
}
...
template <int _r>
typename all_arrays<_r>::iterator::self_type &
all_arrays<_r>::iterator::operator++() {
...
template <int _r>
typename all_arrays<_r>::iterator::reference
    all_arrays<_r>::iterator::operator*() {
  return _array;
}

template <int _r>
typename all_arrays<_r>::iterator::pointer
    all_arrays<_r>::iterator::operator->() {
  return &_array;
}
...
template <int _r> typename all_arrays<_r>::iterator all_arrays<_r>::begin() {
  return iterator(_n);
}

template <int _r> typename all_arrays<_r>::iterator all_arrays<_r>::end() {
  return iterator(-1);
}
...
template <typename T> std::vector<int> index_sort(const T &data) {
  ...
  std::sort(mapping.begin(), mapping.end(),
            [&data](int lhs, int rhs) { return data[lhs] < data[rhs]; });
  return mapping;
}

std::vector<int> inverse_mapping(const std::vector<int> &d);

template <typename T> std::vector<int> rank(const T &data) {
...
template <typename T>
std::vector<T> flatten(const std::vector<std::vector<T>> &s) {
  std::vector<T> t;
  for (const auto &u : s)
...
template <typename T> int polynomial_evaluation(const T &coeff, int x) {
  int r = 0;
  for (int c : coeff)
    r = c + x * r;
  return r;
...
template <typename T> class any_const_iterator {
  ...
  template <typename Iter> struct model : concept {
    ...
    bool equal(const void *rp) const override {
      return _iter == static_cast<const model *>(rp)->_iter;
    }
...
public:
  ...
  any_const_iterator(Iter iter) : _impl(std::make_unique<model<Iter>>(iter)) {
    // static_assert(std::is_same<iter::value_type, value_type>::value,
    // "iterator doesn't iterate over required type");
  }
  ...
  const value_type &operator*() const { return _impl->deref(); }
  ...
  bool operator==(const any_const_iterator &r) const {
    return _impl->type() == r._impl->type() and
           _impl->equal(r._impl->address());
  }

  bool operator!=(const any_const_iterator &r) const { return not(*this == r); }
};

template <typename T> size_t largest(const T &S) {
...
template <typename T> std::deque<T> split(const T &S, size_t k) {
  size_t n = S.size();
  size_t p = n % k;
  size_t q = n / k;
  T U;
...
template <typename T> size_t intersection_size(const T &a, const T &b) {
  auto i = b.cbegin();
  size_t r = 0;
  for (int x : a) {
    while ((*i) < x)
