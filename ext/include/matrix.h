...
template <typename T> int matrix<T>::width() const { return w; }

template <typename T> int matrix<T>::height() const { return h; }
...
template <typename T>
typename std::vector<T>::const_reference matrix<T>::at(int row,
                                                       int column) const {
  return data.at(row * width() + column);
}

template <typename T> void matrix<T>::resize(int height, int width) {
  h = height;
  w = width;
  clear();
}
...
template <typename T> void matrix<T>::clear() { data.assign(w * h, 0); }

template <typename T> matrix<T>::matrix() { w = h = 0; }

template <typename T> matrix<T>::matrix(int height, int width) {
  resize(height, width);
}
