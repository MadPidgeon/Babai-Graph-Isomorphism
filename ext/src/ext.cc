...
std::ostream &operator<<(std::ostream &os, Empty) { return os << "Ø"; }

range::iterator::iterator(int i) : _i(i) {}

range::iterator::iterator(const self_type &other) { _i = other._i; }
...
range::iterator::reference range::iterator::operator*() { return _i; }

range::iterator::pointer range::iterator::operator->() { return &_i; }

bool range::iterator::operator==(const self_type &rhs) { return _i == rhs._i; }

bool range::iterator::operator!=(const self_type &rhs) { return _i != rhs._i; }

size_t range::size() const { return _m - _n; }

range::iterator range::begin() { return iterator(_n); }

range::iterator range::end() { return iterator(_m); }
...
all_tuples::iterator all_tuples::begin() const { return iterator(_n, _r); }

all_tuples::iterator all_tuples::end() const { return iterator(-1, _r); }

all_tuples::all_tuples(int n, int r) : _n(n), _r(r) {}

all_ordered_tuples::iterator::iterator(int n, int r)
    : _n(n), _r(r), _tuple(_r) {
  for (int i = 0; i < _r; i++)
    _tuple[i] = i;
}
...
all_ordered_tuples::iterator::self_type
all_ordered_tuples::iterator::operator++(int) {
  self_type i = *this;
  ++(*this);
  return i;
}
...
all_ordered_tuples::iterator::self_type &
all_ordered_tuples::iterator::operator++() {
  if (_tuple[_r - 1] < _n - 1)
    _tuple[_r - 1]++;
  else {
    int i = _r - 1;
...
all_ordered_tuples::iterator::reference
    all_ordered_tuples::iterator::operator*() {
  return _tuple;
}

all_ordered_tuples::iterator::pointer
    all_ordered_tuples::iterator::operator->() {
  return &_tuple;
}
...
size_t all_ordered_tuples::size() const { return binom(_n, _r); }

size_t all_ordered_tuples::arity() const { return _r; }

range all_ordered_tuples::parent_set() const { return range(0, _n); }

all_ordered_tuples::all_ordered_tuples(int n, int r) : _n(n), _r(r) {}

all_different_tuples::iterator::iterator(int n, int r)
    : _n(n), _r(r), _tuple(_r) {
  for (int i = 0; i < _r; i++)
    _tuple[i] = i;
}
...
all_different_tuples::iterator::self_type
all_different_tuples::iterator::operator++(int) {
  self_type i = *this;
  ++(*this);
  return i;
}
...
all_different_tuples::iterator::self_type &
all_different_tuples::iterator::operator++() {
  if (_r == 0) {
    _n = -1;
    return *this;
  }
...
all_different_tuples::iterator::reference
    all_different_tuples::iterator::operator*() {
  return _tuple;
}

all_different_tuples::iterator::pointer
    all_different_tuples::iterator::operator->() {
  return &_tuple;
}
...
size_t all_different_tuples::arity() const { return _r; }
...
bool bipartiteMatching_Subroutine(const matrix<bool> &M, int p,
  ...
  for (int q : range(0, M.height())) {
    if (M.at(p, q) and not seen.at(q)) {
      seen.at(q) = true;
      if (match.at(q) < 0 or
          bipartiteMatching_Subroutine(M, match.at(q), seen, match)) {
        match.at(q) = p;
        return true;
      }
    }
