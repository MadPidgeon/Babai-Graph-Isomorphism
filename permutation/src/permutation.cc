...
int Permutation::degree() const { return _map.size(); }
...
int Permutation::operator()(int k) const { return _map[k]; }

const std::vector<int> &Permutation::getArrayNotation() const { return _map; }
...
all_permutations::iterator::iterator(const self_type &other)
    : _n(other._n), _p(other._p) {}

all_permutations::iterator::self_type
all_permutations::iterator::operator++(int) {
  self_type i = *this;
  ++(*this);
  return i;
}
...
all_permutations::iterator::self_type &
all_permutations::iterator::operator++() {
...
all_permutations::iterator all_permutations::begin() { return iterator(_n); }

all_permutations::iterator all_permutations::end() { return iterator(-1); }
